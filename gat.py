    ...
    def inference(self, g, device, batch_size=128, emb=True):
        ...
        for l, layer in enumerate(self.gat_layers):
            if l < self.num_layers - 1:
                y = torch.zeros(g.num_nodes(), self.num_hidden * num_heads if l != len(self.gat_layers) - 1 else self.num_classes).to(device)
            else:
                if emb == False:
                    y = torch.zeros(g.num_nodes(), self.num_hidden if l != len(self.gat_layers) - 1 else self.num_classes).to(device)
                else:
                    
                    y = torch.zeros(g.num_nodes(), self.out_dim * num_heads_out).to(device)
